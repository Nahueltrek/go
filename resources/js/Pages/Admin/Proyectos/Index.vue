...
function filterBy(status) {
  router.get('/admin/proyectos', { status }, { preserveState: true })
}

function destroy(p) {
  if (!confirm(`¿Eliminar "${p.name}"? Esta acción no se puede deshacer.`)) return
  router.delete(`/admin/proyectos/${p.id}`, { preserveScroll: true })
}

function approve(p) {
  router.post(`/admin/proyectos/${p.id}/approve`, {}, { preserveScroll: true })
}

function reject(p) {
  router.post(`/admin/proyectos/${p.id}/reject`, {}, { preserveScroll: true })
}
</script>
...
<template>
  ...
  <div class="px-4 py-6">
    <div class="relative -mx-4 px-4 pt-6 pb-8 mb-5 overflow-hidden bg-gradient-to-br from-sky-800 to-sky-950">
      <h1 class="text-xl font-bold text-white animate-fade-in-up">Proyectos</h1>
    ...
    <div class="flex items-center justify-between mt-4 mb-4 gap-2">
      ...
      <Link href="/admin/proyectos/nuevo"
            class="shrink-0 text-xs font-medium rounded-full bg-sky-900 text-white px-4 py-2">+ Nuevo</Link>
    </div>

    <div class="space-y-3">
      <div v-for="(p, i) in projects.data" :key="p.id"
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            ...
            <p class="text-xs text-sky-500">{{ p.organization ?? 'Sin operador' }} · {{ categoryLabel[p.category] ?? p.category }}</p>
          </div>
          <span
        ...
        <div class="flex gap-2 mt-3" v-if="p.status === 'pending_review'">
      ...
      <p v-if="!projects.data?.length" class="text-sm text-sky-400 text-center py-12">
